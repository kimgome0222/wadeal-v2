    ...
    <div className="space-y-3">
      <div className="rounded-xl border border-wadeal-line bg-white p-4">
        <div className="flex items-center justify-between gap-3">
          <p className="text-sm font-black text-wadeal-ink">인증 상태</p>
          <span
            className={`rounded-full px-2.5 py-1 text-[10px] font-black ${getVerificationStatusBadgeClass(verificationStatus)}`}
          >
            {getVerificationStatusLabel(verificationStatus)}
          </span>
        </div>
        <p className="mt-2 text-xs font-bold leading-relaxed text-wadeal-muted">
          결제·배송·환불·고객센터 처리를 위해 주문자 정보를 정확히 입력해 주세요.
        </p>
      </div>

      {profile?.email ?
        <div className="rounded-xl border border-wadeal-line bg-white p-4">
          <p className="text-xs font-black text-wadeal-muted">이메일</p>
          <p className="mt-1 break-all text-sm font-bold text-wadeal-ink">{profile.email}</p>
          {profile.providerLabel ?
            <p className="mt-1 text-xs font-bold text-wadeal-muted">
              {profile.providerLabel} 계정으로 로그인
            </p>
          : null}
        </div>
      : null}

      <form className="space-y-4 rounded-xl border border-wadeal-line bg-white p-4" onSubmit={handleSave}>
        <div>
        ...
        <div>
          ...
          : phoneVerifiedLocally || profile?.phoneVerifiedAt ?
            <p className="mt-2 text-[11px] font-bold text-green-700">휴대폰 인증 완료</p>
          : null}
        </div>
        ...
        <label className="flex cursor-pointer items-start gap-2">
          <input
            ...
            type="checkbox"
          />
          <span className="text-xs font-bold leading-relaxed text-wadeal-muted">
