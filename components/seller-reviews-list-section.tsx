...
export function SellerReviewsListSection({
  sellerName,
  reviews,
  initialVisible = 3,
}: SellerReviewsListSectionProps) {
    ...
    >
      <div>
        ...
        <p className={`mt-1 ${ds.type.caption}`}>
          {sellerName} 판매자에 대한 최근 구매자 후기입니다.
        </p>
      </div>
      ...
      : <>
          <div className="space-y-2">
            {visibleReviews.map((review) => (
              ...
              >
                ...
                <p className={`mt-0.5 ${ds.type.caption} font-medium text-wadeal-ink`}>
          ...
          {hasMore && !expanded ?
            <button
              ...
              type="button"
            >
              판매자 리뷰 더보기 ({reviews.length - initialVisible}개)
            </button>
          : null}
        </>
      }
