  ...
  return (
    <div className="no-scrollbar flex gap-2 overflow-x-auto pb-0.5">
      {SEARCH_PRODUCT_SORT_OPTIONS.map((option) => {
        const active = displaySort === option.value;
        return (
          <button
            aria-pressed={active}
            className={`relative z-10 h-9 shrink-0 cursor-pointer rounded-[18px] px-3.5 text-[13px] font-medium transition-colors duration-[120ms] ease-out active:scale-[0.98] ${
              active ?
