...
export function CartAddedBottomSheet() {
  ...
  return (
    <>
      <button
        aria-label="닫기"
        className="fixed inset-0 z-[220] bg-black/16"
        onClick={closeSheet}
        type="button"
      />
      <div
        aria-label="장바구니 담기 완료"
        className="cart-added-bottom-sheet fixed inset-x-0 bottom-0 z-[221] mx-auto max-w-[430px] overflow-y-auto rounded-t-[20px] bg-white px-6 pb-[max(env(safe-area-inset-bottom),20px)] pt-3 shadow-[0_-8px_24px_rgba(0,0,0,0.08)]"
