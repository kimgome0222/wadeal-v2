-- Wadeal development catalog reset and seed
-- Run this entire file once in the Supabase SQL Editor.
-- Drops and recreates only the catalog tables (products, group_buy_deals, price_tiers).

-- 1. Drop catalog tables (dependency-safe order)
DROP TABLE IF EXISTS price_tiers CASCADE;
DROP TABLE IF EXISTS group_buy_deals CASCADE;
DROP TABLE IF EXISTS products CASCADE;

-- 2. Recreate catalog tables
...
-- 3. Row Level Security (public SELECT only)
...
-- 4. Seed catalog (mirrors lib/deals.ts mock data)
