...
CREATE TABLE payment_methods_mock (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  label TEXT,
...
CREATE INDEX idx_payment_methods_mock_user_id ON payment_methods_mock(user_id);
CREATE INDEX idx_orders_mock_user_id ON orders_mock(user_id);

-- Prototype user for mock participation before auth is wired
INSERT INTO users (id, nickname, email)
VALUES ('00000000-0000-4000-8000-000000000001', '김가나', 'prototype@wadeal.local')
ON CONFLICT (id) DO NOTHING;

-- Seed data mirrors lib/deals.ts mock catalog (slug is the stable URL identifier)
